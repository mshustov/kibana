...
test('returns config at path as observable', async () => {
  ...
  configService.setValidationSchemas(stringSchemaFor('key'));
...
test("returns undefined if fetching optional config at a path that doesn't exist", async () => {
  ...
  const configService = new ConfigService(config$, defaultEnv, logger);
  configService.setValidationSchemas(stringSchemaFor('unique-name'));
...
test('returns observable config at optional path if it exists', async () => {
  ...
  const configService = new ConfigService(config$, defaultEnv, logger);
  configService.setValidationSchemas(stringSchemaFor('value'));
...
test("throws error if 'schema' is not defined for a key", async () => {
  ...
  const config$ = new BehaviorSubject(new ObjectToConfigAdapter({ key: 'value' }));
  const configService = new ConfigService(config$, defaultEnv, logger);
  configService.setValidationSchemas(stringSchemaFor('no-key'));
  const configs = configService.atPath('key', ExampleClass as any);
...
test("throws error if 'setValidationSchemas' was set several times", async () => {
  const config$ = new BehaviorSubject(new ObjectToConfigAdapter({ key: 'value' }));
  const configService = new ConfigService(config$, defaultEnv, logger);
  const performSet = () => configService.setValidationSchemas(stringSchemaFor('key'));

  performSet();
  expect(performSet).toThrowErrorMatchingInlineSnapshot(
    `"Validation schemas have been already set"`
  );
});
...
test('correctly passes context', async () => {
  ...
  configService.setValidationSchemas(new Map([['foo', schemaDefinition]]));
